<template>
  <div class="fridge-view">
      ...
      <template v-if="selectedUnit">
        ...
        <div class="card mb-4">
          <div class="card-header">
            ...
            <span class="badge badge-neutral">{{ unitTypeLabel(selectedUnit.type) }}</span>
          </div>
    ...
    <div v-else class="text-muted text-sm">Ingen kjøleenheter funnet.</div>
  </div>
</template>
...
// ─── Units (FRIDGE + COOLER) ───────────────────────────────────────────────
const coldUnits = computed(() => [
  ...unitsStore.getByType('FRIDGE'),
  ...unitsStore.getByType('COOLER'),
])

const selectedUnitId = ref<number | null>(null)
const selectedUnit = computed(
  () => coldUnits.value.find(u => u.id === selectedUnitId.value) ?? null
)

function unitTypeLabel(type: UnitType): string {
  return type === 'FRIDGE' ? 'Kjøleskap' : 'Kjølerom'
}
...
onMounted(async () => {
  ...
  if (coldUnits.value.length) {
    selectUnit(coldUnits.value[0].id)
  }
})

watch(coldUnits, (units) => {
